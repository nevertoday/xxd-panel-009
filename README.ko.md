<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 009 프로젝트 배너" width="1200">
</p>

<div align="center">

# 🦁 XXD Panel 009｜하프톤 앵커 프린트

### 사진을 작은 하나의 앵커와 넓은 여백, 촘촘함에서 성김으로 흐르는 망점 공간시로

[![Codex Skill](https://img.shields.io/badge/Codex-Skill-000000?style=flat-square)](./SKILL.md)
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#네-가지-출력-하나의-망점-공간-논리)
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 009 예시 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 009 예시 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 009 예시 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 009 예시 8"></td>
  </tr>
</table>

## 추가 3:4 상하 구성 예시

위 16:9 예시와 같은 네 개의 독립 소스를 완전한 3:4 상하 캔버스로 다시 생성했습니다. 위쪽은 현실 사진을 유지하고 아래쪽은 Panel 009의 원본 프롬프트에 따라 재구성합니다. 영어 문구는 각 사진의 내용·분위기·함의에서만 생성했으며 모든 결과를 개별 검수했습니다.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 009 추가 상하 예시 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 009 추가 상하 예시 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 009 추가 상하 예시 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 009 추가 상하 예시 4"></td>
  </tr>
</table>
...
> 작은 앵커 · 넓은 여백 · 하나의 공간 관계 · 별색 체계 · 하프톤 실크스크린

XXD Panel 009는 Codex와 호환 에이전트를 위한 이미지 생성 Skill입니다. 원본 사진의 주제, 윤곽, 자세와 서사 관계를 고정하고 복잡한 정보를 작지만 한눈에 알아볼 수 있는 하나의 시각적 앵커로 압축합니다. 방향과 공간 관계를 하나씩만 선택하고, 넓은 종이색 여백이 거리·공기·멈춤·시간을 맡게 합니다.
...
**Panel 009**은 원본 사진의 핵심을 보존하면서 다음과 같은 시각 방향을 원하는 작업에 적합합니다: **사진을 작은 하나의 앵커와 넓은 여백, 촘촘함에서 성김으로 흐르는 망점 공간시로**. 먼저 용도와 원하는 결과를 확인한 뒤 구성, 크기, 텍스트와 전달 형식을 결정합니다.
...
> XXD Panel 009으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
## 빠른 판단: XXD Panel 009이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진을 작은 하나의 앵커와 넓은 여백, 촘촘함에서 성김으로 흐르는 망점 공간시로 |
| **한눈에 알아볼 특징** | 작은 앵커 · 넓은 여백 · 하나의 공간 관계 · 별색 체계 · 하프톤 실크스크린 |
...
```text
원본 사실 고정 → 하나의 시각적 앵커로 압축 → 방향과 공간 관계를 하나씩만 선택 → 종이 여백이 거리와 시간을 담당 → 종이／주 잉크／안개 별색 구성 → 모든 전환을 망점 밀도로 제작 → 글자를 공간 구조에 결합
```

무관한 사진으로 바꿔도 앵커, 방향, 공간 관계, 망점의 흐름, 종합 색온도와 문구가 거의 그대로 성립한다면 009가 아닙니다.

## 완성작에서 바로 알아볼 수 있는 스타일 특징

- **작은 앵커 하나:** 최소 세 가지 원본 고유 단서로 주제, 윤곽, 자세, 동작과 관계를 적은 정보 안에서도 식별합니다.
- **방향 하나:** 수평 확장, 수직 연장, 고립된 부유, 바깥으로 소멸, 얕은 곳에서 깊은 곳으로의 후퇴 중 하나만 선택합니다.
- **공간 관계 하나:** 수평선, 경계, 그림자 또는 망점 띠 하나면 충분합니다.
- **여백이 주제:** 종이색이 가장 넓은 면적을 차지하고 거리, 공기, 멈춤, 시간 또는 고독을 맡습니다.
- **별색 2–3개:** 종이 바탕, 조용하고 무게감 있는 주 잉크, 필요할 때만 더 옅고 회색인 동계열 안개층입니다.
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 009의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-009 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-009 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-009 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-009 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-009 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-009 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 009 적용, 공통 설정은 한 번만 확정
/xxd-panel-009 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 `~/.claude/skills/xxd-panel-009`에 연결할 수 있습니다. 설치 후 에이전트 세션을 다시 시작하세요.
...
- [Skill 워크플로](SKILL.md)
- [중문 런타임 어댑터](references/xxd-panel-009-prompt.zh-CN.md)
- [영문 런타임 어댑터](references/xxd-panel-009-prompt.en.md)
...
**복잡함은 사진에 남기고, 거리와 멈춤과 메아리는 작은 한 점과 한 장의 종이에 맡깁니다.**
